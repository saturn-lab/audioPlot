...
def npToWav(data, fname):
    ...
    F[4:8] = np.fromstring(np.array([len(data)*4 + itr - 4], dtype=np.uint32).tostring(), dtype=np.uint8)
    F[22:24] = np.fromstring(np.array([1], dtype=np.uint16).tostring(), dtype=np.uint8)
    F[24:28] = np.fromstring(np.array([44100], dtype=np.uint32).tostring(), dtype=np.uint8)
    F[28:32] = np.fromstring(np.array([44100*4], dtype=np.uint32).tostring(), dtype=np.uint8)
    F[32:34] = np.fromstring(np.array([4], dtype=np.uint16).tostring(), dtype=np.uint8)
    F[34:36] = np.fromstring(np.array([32], dtype=np.uint16).tostring(), dtype=np.uint8)
    itr += 4
    F[itr:itr + 4] = np.fromstring(np.array([len(data)*4], dtype=np.uint32).tostring(), dtype=np.uint8)
    itr += 4
    print(F.shape - itr)
    print(data.dtype)
